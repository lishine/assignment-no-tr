...
describe('Polygon API Endpoints', () => {
    ...
    describe('PUT /polygons/:id', () => {
        it('should update a polygon successfully', async () => {
            ...
            const response = await request(app).put(`/polygons/${createdPolygon.id}`).send(updateDto)
            const responseBody: ServiceResponse<Polygon> = response.body

            expect(response.statusCode).toBe(StatusCodes.OK)
            expect(responseBody.success).toBe(true)
            expect(responseBody.message).toBe('Polygon updated successfully')
            expect(responseBody.responseObject.id).toBe(createdPolygon.id)
            expect(responseBody.responseObject.name).toBe(updateDto.name)
            expect(responseBody.responseObject.points).toEqual(updateDto.points)
        })

        it('should return not found when trying to update a non-existent polygon', async () => {
            const nonExistentId = 999
            const updateDto: UpdatePolygon = {
                name: 'Updated Polygon',
            }
            const response = await request(app).put(`/polygons/${nonExistentId}`).send(updateDto)
            const responseBody: ServiceResponse<null> = response.body
        ...
        it('should return bad request for invalid update data', async () => {
            ...
            const response = await request(app).put(`/polygons/${createdPolygon.id}`).send(invalidDto)
            const responseBody: ServiceResponse<null> = response.body
